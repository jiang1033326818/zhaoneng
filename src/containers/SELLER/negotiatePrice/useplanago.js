...
import {Layout, Breadcrumb, Row, Form, Table,Modal,Pagination,Divider,LocaleProvider, Col} from 'antd';
import Lheader from '../common/Iheader.js';
import Lfooter from '../../../components/layout/Ifooter.js';
import "./useplanless.less"
import Acharts from '../foreCasting/chart/bilateralForecast';
import Kcharts from '../foreCasting/chart/bilateralHistory';
...
class selleruseplan extends React.Component {
    ...
    showModal2 = (record) => {
        localStorage.setItem("useplanmo",record.monthStr)
        //this.props.history.push("/seller/useplan")
        window.open(api.basepath+"/seller/useplan")
    }
    showModal20 = (record) => {
    ...
    showModal3 = (record) => {
        ...
        let url ='/api/plan/monthly/'+record.yearStr
        api.post(url, headers, params,
            (res) => {
                console.log(res,"用电计划")
                this.setState({
    ...
    showModal5 = (record) => {
        ...
        let url ='/api/plan/company/'+record.yearStr
        api.post(url, headers, params,
            (res) => {
                console.log(res,"用电计划")
                this.setState({
    ...
    Paging2 = (e) => {
        ...
        let url ='/api/plan/company/'+this.state.stateyear
        api.post(url, headers, params,
            (res) => {
                console.log(res,"用电计划")
                this.setState({
    ...
    render() {
        ...
        // const { three, four,  fourYear } = this.state;
        return (
            <Layout className="layout">
                ...
                <Content style={{padding: '0 50px'}}>
                    ...
                    <div style={{margin: '16px 0'}}>
                        <span>您当前的位置:   </span><Breadcrumb.Item className="location">年度统计</Breadcrumb.Item>
                    </div>
                                    ...
                                    <Col span="12">
                                        {/*<Row>*/}
                                            {/*<Col span={8} offset={16} style={{ textAlign: 'center' }}>*/}
                                                {/*<Select defaultValue="gbr" onChange={this.handleThreeChange}>*/}
                                        ...
                                        {/*</Row>*/}
                                        <Row>
                                            <Acharts style={{width:"100%",height:'auto'}} className="pieChart" name1={'双边预测历史'} name2={'双边预测'} ></Acharts>
                                        </Row>
                                    </Col>
                                    <Col span="12">
                                        {/*<Row>*/}
                                            {/*<Col span={24}>*/}
                                                {/*<Select defaultValue="2018" onChange={this.handleFourYearChange} className="monthSel">*/}
                                        ...
                                        {/*</Row>*/}
                                        <Row>
                                            <Kcharts style={{width:"100%",height:'auto'}} className="pieChart" name1={'双边预测历史'} name2={'双边预测'}></Kcharts>
                                        </Row>
                                    </Col>
                                </Row>
