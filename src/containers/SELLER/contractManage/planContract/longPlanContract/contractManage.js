...
export  default class Plan extends React.Component {
  ...
  handleCancel = () => {
	...
	handleDownLoad = (record) => {
		...
		let url='/api/contract/file/download/bilateral/' + contractId;
		axios({
			method: 'GET',
			url: url,
			data: null,
	...
	handleConNum = (params) => {
		...
		let url='/api/contract/plan/summary';
		api.get(url, headers, param,
		  (res) => {
				this.setState({
    		...
    		<Content style={{ padding: '0 50px' }}>
				...
				<div style={{ background: '#fff', padding: 24, minHeight: 760 }} className="content">
					<Row className="topOne">
						<Row>
							<Col span={8}>
								<h3 className="welcomes">长期协议合同总览</h3>
							</Col>
						</Row>
						<Row>
							<Col span={20}>
					...
					<Row className="payLists">
						<Row>
							<h3 style={{marginTop:"20px"}}>长期协议合同列表</h3>
						</Row>
						<Row>
							<Col span={8}>
						...
						<Row>
							...
							<Col className="buttonGroups" span={8}>
								...
								<Button type="primary" onClick={()=>{this.props.history.push(`/seller/addnewcontract`)}}>新建线上合同</Button>
							</Col>
						</Row>
						<Row>
