...
import { Layout, Breadcrumb, Row, Col, Table, Pagination, LocaleProvider } from 'antd';
...
export  default class yearContract extends React.Component {
	...
	toCompany = (record) => {
		this.props.history.push(`/seller/companycontract/${record.year}`);
	}
	toMonth = (record) => {
		this.props.history.push(`/seller/monthcontract/${record.year}`);
	}
	handleTableList = (params) => {
		...
		let url='/api/contract/plan/yearly';
    		...
    		<Content style={{ padding: '0 50px' }}>
				...
				<div style={{ background: '#fff', padding: 24, minHeight: 760 }} className="content">
					...
					<Row className="payList">
						<Row>
							<h3 style={{marginTop:"20px"}}>长期协议统计-年度汇总</h3>
						</Row>
						<Row>
