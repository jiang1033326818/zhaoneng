...
import { Layout, Breadcrumb, Row, Col, Icon, Form, Input, Button, DatePicker, Upload, Radio,message, Spin, LocaleProvider, Timeline, Select, InputNumber, Divider } from 'antd';
...
class customerUploadContract extends React.Component {
  ...
  render() {
		...
		const { spinLoading, authorization, fileName, companyList,JanL, FebL, MarL, AprL, MayL, JuneL, JulyL, AugL, SeptL, OctL, NovL, DecL,JanS, FebS, MarS, AprS, MayS, JuneS, JulyS, AugS, SeptS, OctS, NovS,DecS, bilateralSpread, biddingSpread, yearStr } = this.state;
    return (
      <Layout className="layout">
    		...
    		<Content style={{ padding: '0 50px' }}>
				...
				<div style={{ background: '#fff', padding: 24, minHeight: 1450 }} className="content">
									...
									<Form>
										...
										>
											{getFieldDecorator('partyAName', {
												...
												<Select placeholder="请选择甲方企业名称" className="inputLengths">
													{
														companyList.map((item)=>{
															return (
																<Option value={item.partnerCompanyName+"-"+item.partnerCompanyId} key={item.id}>{item.partnerCompanyName}</Option>
															)
														})
													}
												</Select>
										...
										<FormItem
											...
											// style={{width:"30%",border:"red",left:"14.65%"}}
										>
											{getFieldDecorator('upload', {
												...
												<LocaleProvider locale={zhCN} >
													<Upload name='file'
														action="http://39.104.91.74:8080/api/contract/file/upload"
										...
										<Row className="buttonGroups">
											...
											>
												...
												<Button className="calBtns" onClick={()=>{this.props.history.push(`/seller/contractmanage`)}}>取消</Button>
											</FormItem>
										</Row>
									</Form>
								</Spin>
