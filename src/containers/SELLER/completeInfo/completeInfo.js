...
render() {
	const { location, company, status, activeKey, current, cid, name, totalCapacity, regionName, natureName,  type,industryName } = this.state;
  return (
														...
														<Card className="cardArea">
															<Row>
																...
																<Col span={12} className="cardItem">
																	<p>企业名称: {company?company:''}</p>
																	<p>行政区划: {regionName?regionName:''} </p>
																	<p>行业分类: {industryName?industryName:''}</p>
																	<p>注册资本:  {totalCapacity?totalCapacity+'万元':''}</p>
																</Col>
																<Col span={12} className="cardItem">
																	<p>公司类型: { type? type:''} </p>
																	<p>企业性质: {natureName?natureName:''} </p>
																	<p>户号:  {cid?cid:''}</p>
																	<p>户名: {name?name:''}</p>
																</Col>
															</Row>
